...
### GENES ARE RANKED BY ENHANCEMENT IN A SINGLE ANCESTRY AND VARIATION IN THE TRAINING DATA ### 
#######################################################################
...
order_frequencies <- function(include_genes, exome_file, expression){
  ...
  no_sex <- exome_file[exome_file$gene %in% include_genes,]  
  keep.num <- 30
  upper.bound <- 1 
  lower.bound <- 0.001
  ...
  nfe_swe.keep <- na.omit(nfe_swe.snps.k$gene) 
